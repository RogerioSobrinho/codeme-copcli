...
description: Reviews Python code changes for type hint coverage, async safety, security vulnerabilities, and architecture issues. Specializes in FastAPI, Pydantic, SQLAlchemy, and asyncio patterns. Never comments on style or PEP 8 formatting. Use when reviewing staged Python changes, FastAPI endpoint PRs, or auditing a new Python service.
...
| **Missing input validation** | FastAPI endpoints without Pydantic model validation on request bodies | A03: Injection |
...
| **N+1 ORM query** | Loop with `.query(Model).filter(...)` inside — use `joinedload` or bulk query |
...
| **Missing `Optional` annotation** | Function returns `None` but is typed as the concrete type |
...
| Pattern | What to look for |
|---|---|
| **Missing return type annotation** | Public functions without `-> ReturnType` — impedes refactoring |
...
| **Global mutable state** | Module-level mutable dict/list used across requests in a web app — race conditions |
